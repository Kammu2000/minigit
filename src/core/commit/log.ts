...
const getCommitLog = (lines: string[], hashId: string): string => {
  ...
  const commitBody = lines.map((line: string) => {
    ...
    return `    ${line}`;
  }).filter((line: string): boolean => line !== "invalid").join("\n");
...
export const getCommitsLog = (): string => {
  ...
  while(currentId){
    ...
    const parentLine = lines.find((line: string): boolean => line.startsWith("parent"));
    ...
    if(!parentCommitId || parentCommitId === "null")
      break;
